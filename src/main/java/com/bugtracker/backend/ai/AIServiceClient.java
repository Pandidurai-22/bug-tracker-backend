...
public class AIServiceClient {
    ...
    public AIServiceClient(
        ...
        logger.info("AIServiceClient initialized with URL: {} and timeout: {}ms", aiServiceUrl, timeout);
    }
    ...
    public ComprehensiveAnalysisResponse getComprehensiveAnalysis(String bugDescription) {
        String url = aiServiceUrl + "/analyze/comprehensive";
        try {
            logger.debug("Calling AI service at: {}", url);
            ComprehensiveAnalysisRequest request = new ComprehensiveAnalysisRequest(bugDescription);
            HttpHeaders headers = new HttpHeaders();
            headers.setContentType(MediaType.APPLICATION_JSON);
            HttpEntity<ComprehensiveAnalysisRequest> entity = new HttpEntity<>(request, headers);
            ...
            if (response.getStatusCode().is2xxSuccessful() && response.getBody() != null) {
                logger.debug("AI service returned successful response with {} tags", 
                    response.getBody().getTags() != null ? response.getBody().getTags().size() : 0);
                return response.getBody();
            } else {
                logger.warn("AI service returned non-success status: {}", response.getStatusCode());
            }
        } catch (HttpClientErrorException e) {
            logger.error("HTTP error calling AI service at {}: {} - {}", url, e.getStatusCode(), e.getResponseBodyAsString());
        } catch (RestClientException e) {
            logger.error("Connection error calling AI service at {}: {}", url, e.getMessage());
        } catch (Exception e) {
            logger.error("Unexpected error calling AI service at {}: {}", url, e.getMessage(), e);
        }
        return null;
    }
